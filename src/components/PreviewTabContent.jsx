...
const ContrastBadge = ({ level, ratio, isPassing }) => {
  ...
  return (
    <div
      className={`flex items-center space-x-2 px-3 py-1.5 rounded-full text-sm font-medium ${bgColor} ${textColor}`}
    >
      <Icon className="h-4 w-4" />
      <span>
        {level}: {ratio.toFixed(2)} {isPassing ? "(Passe)" : "(Échoue)"}
      </span>
    </div>
  );
};
...
const MockupDisplay = ({ type, palette, theme, fontStyle }) => {
  ...
  const textOnPrimary =
    contrast.hex(primaryColor, "#FFFFFF") >
    contrast.hex(primaryColor, "#000000")
      ? "#FFFFFF"
      : "#000000";
    ...
    case "businessCard":
      mockupContent = (
        <div
          ...
          className="flex flex-col justify-center items-center text-center transform scale-90 md:scale-100"
        >
          ...
          >
            <p className="text-xs" style={{ color: textOnPrimary }}>
              contact@example.com
            </p>
            <p className="text-xs" style={{ color: textOnPrimary }}>
              +123 456 7890
        ...
        >
          ...
          <div
            style={{
              ...
              color:
                contrast.hex(secondaryColor, "#FFFFFF") >
                contrast.hex(secondaryColor, "#000000")
                  ? "#FFFFFF"
                  : "#000000",
            }}
  ...
  return (
    <div className="flex justify-center items-center p-4 bg-muted rounded-lg min-h-[200px] md:min-h-[320px]">
      {mockupContent}
    </div>
  );
};
...
const PreviewTabContent = ({
  ...
  const simulatedPreviewColor = simulateColorBlindness(
    currentPreviewColor,
    simulationType
  );

  const textColorOnBackground = theme === "dark" ? "#E2E8F0" : "#1E293B";
  const simulatedTextColor = simulateColorBlindness(
  ...
  const backgroundColor =
    theme === "dark"
      ? simulateColorBlindness("#1E293B", simulationType)
      : simulateColorBlindness("#F8FAFC", simulationType);
  const cardBgColor =
    theme === "dark"
      ? simulateColorBlindness("#2C3E50", simulationType)
      : simulateColorBlindness("#FFFFFF", simulationType);
  const textOnPrimary =
    contrast.hex(primaryColor, "#FFFFFF") >
    contrast.hex(primaryColor, "#000000")
      ? "#FFFFFF"
  ...
  const textOnBackground =
    contrast.hex(backgroundColor, "#FFFFFF") >
    contrast.hex(backgroundColor, "#000000")
      ? "#FFFFFF"
      : "#000000";
  const textOnCard =
    contrast.hex(cardBgColor, "#FFFFFF") > contrast.hex(cardBgColor, "#000000")
      ? "#FFFFFF"
      : "#000000";
        ...
        <Card className="bg-card border-border shadow-xl backdrop-blur-md">
          <CardHeader>
            <CardTitle className="text-2xl text-primary flex items-center">
              <PaletteIcon className="mr-3 h-7 w-7" />
          ...
          <CardContent className="space-y-8">
            <div>
              <h4 className="text-lg font-semibold text-foreground mb-2 flex items-center">
                ...
                Couleur principale pour l'aperçu :
              </h4>
                ...
                {simulatedPalette.map((color, index) => (
                  <motion.button
                    ...
                    className={`w-8 h-8 rounded-full border-2 ${
                      currentPreviewColor === palette[index]
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  >
                    <FontSize className="mr-2 h-4 w-4 text-primary" />
                ...
                <div>
                  <Label
                    htmlFor="lineHeightSlider"
                    className="flex items-center mb-1"
                  >
                    <Baseline className="mr-2 h-4 w-4 text-primary" />
            ...
            >
              ...
              <p style={{ color: textColorOnBackground }}>
                Ceci est un paragraphe d'exemple utilisant la police, la taille
                et l'interligne sélectionnés. Il utilise également une couleur
                de la palette pour le texte sur le fond actuel. Voyez comme la
                lisibilité est affectée par vos choix.
              </p>
            </div>
            ...
            <div>
              <h4 className="text-lg font-semibold text-foreground mb-3 flex items-center">
                ...
                Création de Mockups Automatiques (Concept)
              </h4>
              <Tabs
                defaultValue="businessCard"
                onValueChange={setActiveMockup}
                className="w-full"
              >
                <TabsList className="grid w-full grid-cols-3 mb-4">
                  <RadixTabsTrigger value="businessCard">
                    Carte Visite
                  </RadixTabsTrigger>
                  <RadixTabsTrigger value="websiteHeader">
                    Header Site
                  </RadixTabsTrigger>
            ...
            <div>
              <h4 className="text-lg font-semibold text-foreground mb-3">
                Mini-Maquette de Site (Responsive)
              </h4>
              ...
              <div
                className="mx-auto overflow-hidden border rounded-lg bg-muted/10"
                style={{
              ...
              >
                ...
                >
                  <header
                    className={`py-4 text-center rounded-t-lg preview-hero`}
                    style={{
                      backgroundColor: primaryColor,
                      color: textOnPrimary,
                    }}
                  >
                    <h1
                      className={`${mobileSpecificStyles.heroTitleSize} font-bold`}
                    >
                      Titre Héroïque
                    </h1>
                    <p className={`mt-1 ${mobileSpecificStyles.heroTextSize}`}>
                      Un slogan accrocheur.
                    </p>
                    <Button
                      className={`mt-2 preview-button-cta ${mobileSpecificStyles.heroButtonPadding}`}
                      style={{
                        backgroundColor: secondaryColor,
                  ...
                  <main
                    className={`${mobileSpecificStyles.mainPadding} grid ${
                      previewDevice === "mobile"
                        ? "grid-cols-1"
                        : "grid-cols-1 md:grid-cols-3"
                    } ${mobileSpecificStyles.mainGap}`}
                  >
                    {[1, 2, 3].map((i) => (
                      <Card
                        key={i}
                        className={`shadow-lg preview-card ${mobileSpecificStyles.cardPadding}`}
                        style={{
                      ...
                      >
                        <CardHeader
                          className={`${mobileSpecificStyles.cardHeaderPadding}`}
                        >
                          <CardTitle
                            className={`preview-card-title ${mobileSpecificStyles.cardTitleSize}`}
                            style={{
                              color: safePaletteColor(
                                i % simulatedPalette.length,
                                primaryColor
                              ),
                            }}
                          >
                        ...
                        <CardContent
                          className={`${mobileSpecificStyles.cardContentPadding}`}
                        >
                          <p className={`${mobileSpecificStyles.cardTextSize}`}>
                            Contenu de la carte.
                          </p>
                          <Button
                            variant="outline"
                            className={`mt-2 w-full preview-button-outline ${mobileSpecificStyles.cardButtonPadding}`}
                            style={{
                              borderColor: safePaletteColor(
                                (i + 1) % simulatedPalette.length,
                                secondaryColor
                              ),
                              color: safePaletteColor(
                                (i + 1) % simulatedPalette.length,
                  ...
                  <footer
                    className={`text-center py-2 preview-footer ${mobileSpecificStyles.footerTextSize}`}
                    style={{
                      backgroundColor: safePaletteColor(
                        simulatedPalette.length - 1,
                        primaryColor
                      ),
                      color:
                        contrast.hex(
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mb-3">
                Texte ({simulatedTextColor}) sur fond de la couleur principale
                sélectionnée ({simulatedPreviewColor}).
              </p>
              <div className="flex flex-col sm:flex-row gap-3">
                <ContrastBadge
                  level="AA (Normal)"
                  ratio={contrastRatio}
                  isPassing={wcagAAPasses}
                />
                <ContrastBadge
                  level="AAA (Normal)"
                  ratio={contrastRatio}
                  isPassing={wcagAAAPasses}
                />
              </div>
